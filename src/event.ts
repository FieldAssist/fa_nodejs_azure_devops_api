...
export async function getEpics(orgUrl: string, token: string): Promise<WorkItem[]> {
  try {
    ...
    const query = `Select [System.Id], [System.Title], [System.State], [System.Description] From WorkItems Where [System.WorkItemType] = 'Epic' AND [System.TeamProject] = 'Field_Assist' AND [State] NOT IN ('Removed') order by [Microsoft.VSTS.Common.Priority] asc, [System.CreatedDate] desc`;
    const backlogRefList = await workItemTrackingApi.queryByWiql({ query: query })
    const backlogIds = backlogRefList?.workItems?.map(value => value.id) ?? [];
    // @ts-ignore
